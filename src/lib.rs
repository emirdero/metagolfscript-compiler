                ...
                loop {
                    match chars.peek() {
                        ...
                        Some(_) | None => {
                            ...
                            block_items.push(if item.is_ok() {
                                item.unwrap()
                            } else {
                                return Some(item);
                            });
...
impl Interpreter {
    ...
    pub fn exec_items(&mut self, items: &[Item]) -> Result<&[Item], GSError> {
        for item in items.iter().cloned() {
            match item {
                ...
                x @ Item::Num(_) | x @ Item::Str(_) | x @ Item::Block(_) => self.push(x),
    ...
    fn add(&mut self) -> GSErr {
        match coerce(self.pop2()?) {
            ...
            (Item::Str(x), Item::Str(y)) => self.push(Item::Str(y + &x)),
    ...
    fn not(&mut self) -> GSErr {
        match self.pop()? {
            Item::Num(x) => self.push(Item::Num(if x == 0 { 1 } else { 0 })),

            Item::Str(ref x) => self.push(Item::Num(if x == "" { 1 } else { 0 })),
    ...
    fn mul(&mut self) -> GSErr {
        match self.pop2()? {
            ...
            (Item::Num(y), Item::Str(x)) | (Item::Str(x), Item::Num(y)) => {
                if y < 0 {
                    return Err(GSError::Runtime(
            ...
            (Item::Num(y), Item::Array(x)) | (Item::Array(x), Item::Num(y)) => {
                if y < 0 {
                    return Err(GSError::Runtime(
                        "repeat string value is negative".to_string(),
                    ));
    ...
    fn neg(&mut self) -> GSErr {
        match self.pop()? {
            ...
            Item::Str(ref x) => match self.exec(x) {
                Err(e) => {
                    return Err(GSError::Runtime(format!(
            ...
            Item::Block(ref x) => match self.exec_items(x) {
                Err(e) => {
                    return Err(GSError::Runtime(format!(
                        "invalid expression statement: {:?}",
                        e
    ...
    fn lt(&mut self) -> GSErr {
        match self.pop2()? {
            (Item::Num(y), Item::Num(x)) => {
                self.push(Item::Num(if x < y { 1 } else { 0 }));
            }
            ...
            (Item::Num(x), Item::Array(y)) | (Item::Array(y), Item::Num(x)) => {
                self.push(Item::Array(
                    y.iter()
                        .cloned()
                        .take(x as usize)
    ...
    fn array(&mut self) -> GSErr {
        match self.pop()? {
            Item::Num(x) => {
                self.push(Item::Array(
                    (0..x).map(Item::Num).collect_vec().into_boxed_slice(),
                ));
            }
    ...
    fn builtin_abs(&mut self) -> GSErr {
        match self.pop()? {
            ...
            x @ _ => return Err(GSError::Runtime(format!("invalid type for `abs`: {:?}", x))),
        }
        Ok(())
    }
